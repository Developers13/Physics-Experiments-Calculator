...
- 同一实验的B类不确定度应保持不变
- 依据《标准2017》，对输入量的不确定度评定使用A类评定，仅在A类不结果很小或某个来源不能在重复测量中体现时用B类评定。但现大多数大学物理实验教材对重复测量量使用AB两类判定，再使用方和根合成为总不确定度，与国标有所不同。本计算器使用的是后者方法
- 不确定度传递计算时不考虑输入量之间的相关性

### 计算公式
脚本基于stats.py模块计算数据，以下是主要公式（使用KaTeX语法）：

- **平均值**：$\bar{x} = \frac{\sum_{i=1}^{n} x_i}{n}$
- **平均偏差**：$d_i = x_i - \bar{x}$
- **标准偏差**：$s = \sqrt{\frac{\sum_{i=1}^{n} d_i^2}{n}}$
- **标准误差**：$SE = t \cdot s$，其中 $t$ 根据样本大小 $n$ 和置信水平（默认0.683）从t分布表选取
- **A类不确定度**：$u_A = \frac{SE}{\sqrt{n-1}}$
- **B类不确定度**：高斯分布 $u_B = \frac{\epsilon}{3}$，均匀分布 $u_B = \frac{\epsilon}{1.46}$
- **坏值检测与去除**：若 $|d_i| > 3 \cdot SE$，则将该值视为坏值并移除，重复计算直至无坏值
