...
class Dataset:
    ...
    def recalculate(self, data=None):
        """Recompute bad-value keys and iteratively remove bad values.

        This method updates `self.org` to the cleaned data and returns it.
        It accepts an optional `data` list; when omitted, operates on `self.org`.
        """
        # start from a copy of provided data or the current org
        cleaned = list(data if data is not None else self.org)
        # ensure internal org is in sync
        self.org = [float(x) for x in cleaned]

        # detect and remove bad values until none remain
        self.checkBadValue(self.org)
        while self.bvkey:
            cleaned = self.removeBadValue(self.org)
            self.org = [float(x) for x in cleaned]
            self.checkBadValue(self.org)
